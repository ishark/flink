...
public class TaxiRide {
	public Long tripID      ; // a unique id for each trip
	//public Date time       ; // the start or end time of a trip
	public String event       ; // 'START' or 'END' flag indicating the record type
	public Long startLon    ; // the longitude of the trip start location
	public Long startLat    ; // the latitude of the trip start location
	public Long endLon      ; // the longitude of the trip end location
	public Long endLat      ; // the latitude of the trip end location
	Integer passengerCnt;    // number of passengers
	Double tripDistance; // actual travel distance (-1 in START records)
	
	public TaxiRide(String[] fields) throws ParseException {
		int index = 0;
		this.tripID = Long.parseLong(fields[index++]);
		//SimpleDateFormat format = new SimpleDateFormat("YYYY-MM-DD hh:mm:ss");
		//this.time = (Date)format.parse(fields[index++]);
		this.event = fields[index++];
		this.startLon = Long.parseLong(fields[index++]);
		this.startLat = Long.parseLong(fields[index++]);
		this.endLon = Long.parseLong(fields[index++]);
		this.endLat = Long.parseLong(fields[index++]);
		passengerCnt = Integer.parseInt(fields[index++]);
		tripDistance = Double.parseDouble(fields[index++]);
	}
	@Override
	public String toString() {
		return "{ tripId = " + this.tripID + " event = " + event + "}";
	}
